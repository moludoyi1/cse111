...
def print_death_age(people_dict):
    ...
    for person_key, person_list in people_dict.items():
        ...
        birth = person_list[BIRTH_YEAR_INDEX]
        death = person_list[DEATH_YEAR_INDEX]

        agedeath = death - birth
...
def count_genders(people_dict):
    ...
    """
    males = 0 
    for erson_key, person_list in people_dict.items():
        gender = person_list[GENDER_INDEX]
        if gender == 'M':
            males += 1
    
    people = len(people_dict)
    females = people - males

    print('Genders')
    print(f'Total number of males: {males}')
    print(f'Total number of females: {females}')
...
def print_marriages(marriages_dict, people_dict):
    ...
    for marriage_key, marriage_list in marriages_dict.items():
        ...
        weddingyear = marriage_list[WEDDING_YEAR_INDEX]
        ...
        husbandlist = people_dict[husband]
        husbandname = husbandlist[NAME_INDEX]
        husbandbirth = husbandlist[BIRTH_YEAR_INDEX]

         # Calculate the husband's age when he married.
        husbandage = weddingyear - husbandbirth
        ...
        wifelist = people_dict[wife]
        wifename = wifelist[NAME_INDEX]
        wifebirth = wifelist[BIRTH_YEAR_INDEX]

        # Calculate the wife's age when she married.
        wifeage = weddingyear - wifebirth

        # Print the data about the marriage for the user to see.
        print(f'{husbandname} {husbandage}' \
            f' > {weddingyear} < {wifename} {wifeage}')
